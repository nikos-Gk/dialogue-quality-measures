...
if openAIKEY :
    ...
    for disc in discussions:
        ...
        print("Overall Informativeness Score-Proccessing discussion: ",disc_id," with LLM")
        inform_score=calculate_discussion_informativeness_score(utterances,conv_topic,openAIKEY)
    ...
    for disc_id, turnAnnotations in inform_scores_llm_output_dict.items():
        for label in turnAnnotations:
            if label==-1:
                print("LLM output with missing overall informativeness score , skipping discussion\n")
                print(label)
                continue
    ...
    inform_scores_mean=np.mean([float(number) for number in list(inform_scores_per_disc.values())])
    
    inform_scores_per_disc["aggregate_mean"]=str(inform_scores_mean)
    
    
    with open('inform_scores_per_discussion_'+timestr+'.json', 'w',encoding="utf-8") as fout:
