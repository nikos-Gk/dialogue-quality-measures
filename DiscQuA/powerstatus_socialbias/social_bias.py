...
def calculate_social_bias_labels(utts, topic, openAIKEY):
...
def calculate_social_bias(input_directory, openAIKEY, moderator_flag):
    ...
    if not openAIKEY:
        print(
            "OpenAI API key does not exist. Social bias labels will not be annotated. Exiting"
        )
        sys.exit(1)
    ...
    with open(
        "llm_output_socialbias_per_response_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
    ...
    with open(
        "disc_socialbias_mean_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
        json.dump(average_sb_labes_per_disc, fout, ensure_ascii=False, indent=4)

    with open(
        "socialbias_per_utterance_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
