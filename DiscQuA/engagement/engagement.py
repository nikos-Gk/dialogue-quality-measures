...
def engagement(input_directory, openAIKEY, moderator_flag=True):
    ...
    if not openAIKEY:
        print(
            "OpenAI API key does not exist. Engagement scores will not be computed. Exiting"
        )
        sys.exit(1)
    ...
    with open(
        "llm_output_engagement_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
    ...
    engag_scores_mean = np.mean(
        [float(number) for number in list(engag_scores_per_disc.values())]
    )

    engag_scores_per_disc["aggregate_mean"] = str(engag_scores_mean)

    with open(
        "engag_scores_per_discussion_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
