...
def calculate_diversity(input_directory, openAIKEY, moderator_flag=True):
    ...
    if not openAIKEY:
        print(
            "OpenAI API key does not exist. Diversity scores will not be computed. Exiting"
        )
        sys.exit(1)
    ...
    with open(
        "llm_output_diversity_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
    ...
    div_scores_mean = np.mean(
        [float(number) for number in list(div_scores_per_disc.values())]
    )

    div_scores_per_disc["aggregate_mean"] = str(div_scores_mean)

    with open(
        "div_scores_per_discussion_" + timestr + ".json", "w", encoding="utf-8"
    ) as fout:
