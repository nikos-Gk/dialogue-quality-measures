...
If the new utterance fits multiple categories, list all that apply. Ensure that your final answer clearly identifies whether each of the speech act labels (0-11) are applicable (1) or not applicable (0) to the new utterance.
...
def dialogue_acts(
    ...
    Returns:
        dict: A dictionary mapping discussion IDs to lists of per-utterance dialogue act feature dictionaries.
              Each entry is keyed by message ID key and contains extracted features.
    ...
    save_dict_2_json(
        speechactslabels_llm_output_dict,
        "llm_output_dialogueact_per_response",
        disc_id,
        timestr,
    )

    """
    with open("llm_output_dialogueact_per_response_.json", encoding="utf-8") as f:
    ...
    save_dict_2_json(
        speechact_per_response, "dialogueact_per_utterance", disc_id, timestr
    )
