...
parser.add_argument(
                    "--input_file", help="Input csv file."
                    )
...
def extract_conv_info(matched_df, unique_user_ids,conv_id):
    ...
    if has_moderator and MODERATOR_NAME not in unique_user_ids:
        print("WARNING: has_moderator true but no moderator in user ids, conv_id: ",conv_id)
...
def get_user_prompts(unique_user_ids, df_to_extract_from, conv_id):
    ...
    for user in unique_user_ids:
        ...
        userprompt_unique=extract_info('user_prompt', matcheduser_df, f'WARNING: different prompts for the same user, user_id,conv_id :{(user,conv_id)}'),
...
def get_conv_messages(df_to_extract_from, conv_id):
    ...
    for order in message_order:
        matchedmessageorder_df=filter_df_by_key(order, df_to_extract_from, 'message_order')
       
        turnuser_unq=extract_info('user',matchedmessageorder_df,f'WARNING: different user for the same message: conv_id, turn_id: {(conv_id,order)}')        
        message_unq=extract_info('message',matchedmessageorder_df,f'WARNING: different messages for the same user, conv_id, turn_number: {(turnuser_unq,conv_id,order)}')
        model_unq=extract_info('model',matchedmessageorder_df,f'WARNING: different model for the same user and turn_id: {(conv_id, order)}')
        message_id_unq=extract_info('message_id',matchedmessageorder_df, f'WARNING: different message_id for the same user and turn_id: conv_id,turn_id {(conv_id,order)}')
        message_dict[str(order)]=(turnuser_unq,message_unq,model_unq,str(message_id_unq))
    return message_dict
...
for conv_id in tqdm(unique_conv_ids):
    ...
    time_stamp, ctx_length_conv, conv_variant, llm_model=extract_conv_info(matched_df, unique_user_ids,conv_id)
    ...
    if not os.path.exists(file_path_no_mod) and  MODERATOR_NAME not in unique_user_ids:
