...
After the Chain-of-Thoughts reasoning steps, rate the diversity of the arguments of the entire discussion on a scale of 1 to 5, where 1 is of poor diversity quality (arguments of high similarity) and 5 of high diversity quality (diverse arguments). 
...
if not openAIKEY:
    print("OpenAI API key does not exist. Diversity scores will not be computed. Exiting")    
    sys.exit(1)
...
def calculate_discussion_diversity_score(utts,topic,key):
    conv_text=""
    for utt in utts:
        text=utt.text
...
if openAIKEY :
    ...
    for disc in discussions:
        ...
        div_score=calculate_discussion_diversity_score(utterances,conv_topic,openAIKEY)
    ...
    with open('llm_output_diversity_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(divers_scores_llm_output_dict, fout, ensure_ascii=False, indent=4)
    ...
    for disc_id, turnAnnotations in divers_scores_llm_output_dict.items():
        for label in turnAnnotations:
            if label==-1:
                print("LLM output with missing overall diversity score , skipping discussion\n")
                print(label)
                continue
            parts=label.split("diversity of the arguments of the above discussion is:")
            value=parts[1]
    ...
    div_scores_mean=np.mean([float(number) for number in list(div_scores_per_disc.values())])
    
    div_scores_per_disc["aggregate_mean"]=str(div_scores_mean)
    
    
    with open('div_scores_per_discussion_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(div_scores_per_disc, fout, ensure_ascii=False, indent=4)
