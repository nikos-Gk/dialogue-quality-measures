...
if not openAIKEY:
    print("OpenAI API key does not exist. Toxicity labels will not be computed. Exiting")    
    sys.exit(1)
...
def calculate_response_toxicity_score(utts,topic,key):
    conv_hist = ''
    annotations_ci=[]
...
if openAIKEY :
    ...
    for disc in discussions:
        try:
            ...
            print("Toxicilty Label Per Response-Proccessing discussion: ",disc_id," with LLM")
            tox_per_resp=calculate_response_toxicity_score(utterances,conv_topic,openAIKEY)
    ...
    with open('llm_output_tox_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(tox_per_resp_scores_llm_output_dict, fout, ensure_ascii=False, indent=4)
    ...
    for disc_id, turnAnnotations in tox_per_resp_scores_llm_output_dict.items():
        ...
        for label in turnAnnotations:
            if label==-1:
                print("LLM output with missing toxicity response label , skipping response\n")
                print(label)
                counter+=1
                continue
    ...
    for disc_id, utt_coh in toxicity_scores_per_response.items():
        ...
        utt_coh_per_disc_df[cols] = utt_coh_per_disc_df[cols].apply(pd.to_numeric, errors='coerce')
    ...
    with open('disc_aver_tox_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(average_toxicity_per_disc, fout, ensure_ascii=False, indent=4)
        
    with open('toxicity_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
