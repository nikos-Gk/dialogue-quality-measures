...
The texts above show a dialogue in an online chatroom with respect to a potentially controversial post between two or more individuals.
Post: {post}
All individuals answer to each other by presenting arguments on why they think the post(s) is or isn't reasonable, possibly incorporating 
inflammatory and aggressive speech.
Now, please use chain-of-thought reasoning to evaluate the average overall quality of the arguments presented in the above discussion.
Although, a definition of overall argument quality does not exist so far, try to judge about the
overall quality based on all those argument quality dimensions that you think influence the overall quality of the given argumentation.
After the Chain-of-Thoughts reasoning steps, you should assign a score for the average overall argument quality of the arguments presented in the entire discussion on a scale from 1 to 3, where 1 is low quality, 2 is medium quality  and 3 is high quality. 
Conclude your evaluation with the statement: 'The average quality of the arguments presented in the above discussion is: [X]', where X is the numeric score you've determined. Please, ensure that your last statement is the score in brackets [].
"""
...
class OAQuality():
    ...
    def calculate_ovargquality_scores(self):
        ...
        if mode_prompt=='rating':    
            formatted_prompt=prompt_ratingscale.format(conv_text=conv_text,post=self.conv_topic)
        else:
            formatted_prompt=prompt_realnumber.format(conv_text=conv_text,post=self.conv_topic)
        annotations_ci=[]
        try:
