...
def calculate_dialogue_coherence_score(utts,topic,key):
    conv_text=""
    for utt in utts:
        text=utt.text
...
if openAIKEY :
    ...
    for dialog in dialogs:
        ...
        print("Overall Coherence Score-Proccessing dialog: ",dialog_id," with LLM")
        coh_score=calculate_dialogue_coherence_score(utterances,conv_topic,openAIKEY)
        coherence_scores_llm_output_dict[dialog_id]=coh_score
    ...
    coherence_scores_per_dialog={}
    for dialog_id, turnAnnotations in coherence_scores_llm_output_dict.items():
        for label in turnAnnotations:
            if label==-1:
                print("LLM output with missing overall coherence score , skipping dialogue\n")
                print(label)
                continue
            ...
            coherence_scores_per_dialog[dialog_id]=value
    
    coh_scores_mean=np.mean([float(number) for number in list(coherence_scores_per_dialog.values())])
    
    coherence_scores_per_dialog["aggregate_mean"]=str(coh_scores_mean)
    
    
    with open('coherence_scores_per_dialog_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(coherence_scores_per_dialog, fout, ensure_ascii=False, indent=4)
