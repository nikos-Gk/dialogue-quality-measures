...
if not openAIKEY:
    print("OpenAI API key does not exist. Dialogical labels will not be computed. Exiting")    
    sys.exit(1)
...
def calculate_dialog_labels(utts,topic,key):
...
if openAIKEY :
    ...
    for disc in discussions:
        try:
            ...
            print("Dialogical labels Per Response-Proccessing discussion: ",disc_id," with LLM")
            dial_labels_per_resp=calculate_dialog_labels(utterances,conv_topic,openAIKEY)
    ...
    with open('llm_output_diallabels_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
    ...
    for disc_id, dial_label in dialo_labels_per_response.items():
        ...
        dial_label_per_disc_df[cols] = dial_label_per_disc_df[cols].apply(pd.to_numeric, errors='coerce')
    ...
    with open('disc_aver_diallabel_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
        json.dump(average_diallabel_per_disc, fout, ensure_ascii=False, indent=4)
        
    with open('diallabel_per_response_'+timestr+'.json', 'w',encoding="utf-8") as fout:
