...
with open(os.path.join('lexicons', 'my_geo.txt'),encoding="utf-8") as f:
    lexicons['geo'] = [line.strip().lower() for line in f]

with open(os.path.join('lexicons', 'my_meta.txt'),encoding="utf-8") as f:
    lexicons['meta'] = [line.strip().lower() for line in f]

with open(os.path.join('lexicons', 'my_certain.txt'),encoding="utf-8") as f:
    lexicons['certain'] = [line.strip().lower() for line in f]

with open(os.path.join('lexicons', 'my_hedges.txt'),encoding="utf-8") as f:
...
desired_tags=("NN", "NNP", "NNPS", "NNS", "^","JJ", "JJR","JJS","VB","VBD","VBG","VBN","VBP","VPZ")

def get_content_tagged(words, tags):
    """Return content words based on tag"""
    return [w for w, tag in zip(words.lower().split(), tags.split()) if tag in desired_tags]
...
def message_features(reasons, stopwords=mallet_stopwords):
    ...
    for k, (user, tokens, tags) in enumerate(reasons):
        features = {}
        content_words = [w for w in get_content_tagged(tokens, tags) if w not in stopwords]
