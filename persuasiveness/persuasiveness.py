...
if not openAIKEY:
    print("OpenAI API key does not exist. Persuasiveness scores will not be computed. Exiting")    
    sys.exit(1)
...
if openAIKEY :
    ...
    for disc in discussions:
        ...
        print("Overall Persuasiveness Score-Proccessing discussion: ",disc_id," with LLM")
        pers_score=calculate_discussion_persuasiveness_score(utterances,conv_topic,openAIKEY)
        pers_scores_llm_output_dict[disc_id]=pers_score




    with open('llm_output_persuasiveness_'+timestr+'.json', 'w',encoding="utf-8") as fout:
    ...
    for disc_id, turnAnnotations in pers_scores_llm_output_dict.items():
        for label in turnAnnotations:
            if label==-1:
                print("LLM output with missing overall persuasiveness score , skipping discussion\n")
                print(label)
                continue
            parts=label.split("persuasiveness of the arguments of the above discussion is:")
            value=parts[1]
    ...
    pers_scores_mean=np.mean([float(number) for number in list(pers_scores_per_disc.values())])
    
    pers_scores_per_disc["aggregate_mean"]=str(pers_scores_mean)
    
    
    with open('pers_scores_per_discussion_'+timestr+'.json', 'w',encoding="utf-8") as fout:
